...
export class JssContextService {
  ...
  private pathSource = new BehaviorSubject<string>('Default Path'); // Holds the data
  currentPath = this.pathSource.asObservable(); // Observable for components to subscribe to

  sendPathFile(pathFile: string) {
    this.pathSource.next(pathFile); // Update message
  }
